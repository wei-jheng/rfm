...
group_stats = f'{catalog}.{schema}.group_stats'
group_recency_stats = f'{catalog}.{schema}.group_recency_stats'
group_frequency_stats = f'{catalog}.{schema}.group_frequency_stats'
group_monetary_stats = f'{catalog}.{schema}.group_monetary_stats'
...
def group_stats_mv():
    ...
    df = df.withColumn("group_flag", F.coalesce(F.col("group_flag"), F.lit("全")))
    
    return df.orderBy("group_flag")


@dp.materialized_view(
  name=group_recency_stats,
  cluster_by_auto = True,
)
def group_recency_stats_mv():
    df = spark.read.table(source)
    df = (
        df
        .rollup("group_flag")
...
@dp.materialized_view(
  name=group_frequency_stats,
  cluster_by_auto = True
  )
def group_frequency_stats_mv():
    df = spark.read.table(source)
    df = (
        df
        .rollup("group_flag")
...
@dp.materialized_view(
  name=group_monetary_stats,
  cluster_by_auto = True
  )
def group_monetary_stats_mv():
    df = spark.read.table(source)
    df = (
        df
        .rollup("group_flag")
